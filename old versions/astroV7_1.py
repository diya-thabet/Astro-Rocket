...
# --- PlayerDebris Class (NEW) ---
class PlayerDebris(pygame.sprite.Sprite):
    """ Physics-based debris for player death """
...
# ... (This class is updated) ...
    def __init__(self):
        ...
        self.player_data = self.load_player_data() # NEW
        self.sounds = self.load_sounds()
        self.player = Player() # Default player for menus
...
# ... (This function is unchanged) ...
    ...
    def load_player_data(self): # NEW
        """ Loads total credits and unlocked ships """
        try:
            with open(PLAYER_DATA_FILE, "r") as f:
                data = json.load(f)
                if "unlocked_ships" not in data: # Handle old data
    ...
    def save_player_data(self): # NEW
        """ Saves total credits and unlocked ships """
        try:
...
# ... (This function is unchanged) ...
    ...
    def start_new_game(self, ship_type="Cruiser"): # Updated
        self.level = 1
        self.player = Player(ship_type) # NEW: Use selected ship
    ...
    def spawn_asteroids(self, count, level):
        ...
        is_minefield = False
        is_boss_level = False # NEW
        
        # NEW: Boss Level
        if self.level > 0 and self.level % 5 == 0:
            is_boss_level = True
            count = 0 # No asteroids
            num_bosses = 1 + (self.level // 10) # 1 boss, then 2 at level 10
...
# ... (This function is unchanged) ...
    ...
    def create_player_debris(self): # NEW
        """ Creates physics debris when player dies """
        points = self.player.get_ship_points()
        p1, p2, p3 = points
        
        # Create 3 debris pieces, one for each line of the ship
        ...
        self.debris.add(debris1, debris2, debris3) # Add to debris group for drawing
...
# ... (This function is updated) ...
        while self.running:
            ...
            elif self.game_state == "SHIP_SELECT": self.update_menu() # Use same bg anim
            self.draw()
            self.clock.tick(FPS)
        pygame.quit()
            ...
            if event.type == pygame.QUIT: self.running = False
                ...
                if self.game_state == "PLAYING" and self.level_clear_timer == 0:
                    ...
                    if event.key == pygame.K_RETURN:
                        self.game_state = "SHIP_SELECT" # Go to ship select
                        
                elif self.game_state == "START_MENU":
                    if event.key == pygame.K_RETURN:
                        self.game_state = "SHIP_SELECT" # Go to ship select
                        
                elif self.game_state == "SHIP_SELECT": # NEW
...
# ... (This function is updated) ...
        ...
        # Don't spawn normal UFOs on boss levels
        ...
        if not self.asteroids and not self.ufos and not self.hunter_mines and self.level_clear_timer == 0:
...
# ... (This function is updated) ...
        ...
        for asteroid, bullets_hit in asteroid_hits.items():
            ...
            if is_laser: asteroid.kill()
            else: asteroid.hit_flash_timer = 5; asteroid.kill() # Simpler
        ...
        if self.player.invulnerable_timer == 0 and self.player.near_miss_cooldown == 0:
            for asteroid in self.asteroids:
                ...
                if dist < (asteroid.radius + self.player.size * 0.5):
                    ...
                    hit_occured, is_fatal = self.player.hit() # NEW
                    if hit_occured:
                        self.create_player_debris() # NEW
                        if is_fatal:
...
# ... (This function is updated) ...
        ...
        for i in range(self.player.lives):
            ...
            temp_player.draw(surface) # Use the player's own draw method
...
# ... (This function is unchanged) ...
    ...
    def draw_ship_select(self): # NEW
        """ Draws the Ship Selection screen """
        self.screen.fill(BACKGROUND_COLOR)
...
# ... (This function is updated) ...
    ...
    def draw_background(self, surface): # Renamed
...
# ... (This function is updated) ...
        if self.game_state == "PLAYING":
            ...
            self.draw_background(self.game_surface) # Updated
            ...
            self.draw_ship_select() # Draws on self.screen
