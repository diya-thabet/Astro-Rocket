...
# ... (This class is updated) ...
    ...
    def reset(self):
        ...
        self.flow_text_shake_timer = 0 # NEW: For UI animation
    ...
    def update(self):
        ...
        if self.flow_text_shake_timer > 0: self.flow_text_shake_timer -= 1 # NEW
    ...
    def draw(self, surface):
        ...
        # NEW: Respawn "Warp-In" animation
        respawn_anim_len = 30 # frames
        if self.invulnerable_timer > PLAYER_INVULN_TIME - respawn_anim_len:
            progress = (PLAYER_INVULN_TIME - self.invulnerable_timer) / respawn_anim_len # 0 -> 1
            
            # Draw shrinking warp lines
            center_x, center_y = self.x, self.y
            for i in range(20):
                angle = random.uniform(0, 360)
                rad = deg_to_rad(angle)
                start_dist = (1.0 - progress) * 150 + 20 # 170 -> 20
                end_dist = (1.0 - progress) * 200 + 40 # 240 -> 40
                ...
                alpha = progress * 255 # Fade in
                
                line_surf = pygame.Surface((SCREEN_WIDTH, SCREEN_HEIGHT), pygame.SRCALPHA)
                pygame.draw.line(line_surf, (255, 255, 255, alpha), (start_x, start_y), (end_x, end_y), 2)
                surface.blit(line_surf, (0, 0))
            
            # Don't draw ship if just starting animation
            if progress < 0.2:
                return
...
# ... (This function is unchanged) ...
    ...
    def add_score(self, points, game_sfx):
        ...
        if self.flow_state_timer == 0:
            self.flow_level += 1
            self.flow_text_shake_timer = 15 # NEW: Trigger UI shake
            # Check for trigger
...
# ... (This class is updated) ...
    def __init__(self, x=None, y=None, size=ASTEROID_LARGE_SIZE, game_level=1):
        ...
        self.spawn_timer = 20 # NEW: 20 frame spawn-in
    ...
    def update(self):
        ...
        if self.spawn_timer > 0: # NEW
            self.spawn_timer -= 1 # NEW

    def draw(self, surface):
        points = []
        
        # NEW: Calculate scale based on spawn timer
        scale = 1.0
        if self.spawn_timer > 0:
            # Animate from 0 to 1.0
            scale = 1.0 - (self.spawn_timer / 20.0)

        for i in range(self.num_points):
            angle_step = 360 / self.num_points
            rad = deg_to_rad(i * angle_step + self.rot_angle)
            dist = self.radius * self.shape_offsets[i] * scale # NEW: apply scale
...
# --- UFOElite Class (NEW) ---
class UFOElite(UFO):
    """ A more dangerous UFO that fires a spread shot """
    def __init__(self, game):
        super().__init__(game)
        # Make it faster and more frequent
        self.vel_x *= 1.3
        self.shoot_cooldown = 70 # Shoots faster
        self.health = 2 # Takes 2 hits (or 1 laser)
        self.size = 22 # Slightly larger

    def shoot(self):
        """ Fires a 3-shot spread """
        dx = self.game.player.x - self.x
        dy = self.game.player.y - self.y
        ...
        for angle in angles:
            ...
            self.game.all_sprites.add(new_bullet)
            self.game.enemy_bullets.add(new_bullet)
        # if self.game.sounds["ufo_shoot"]: self.game.sounds["ufo_shoot"].play()

    def draw(self, surface):
        """ Draw as a red, more menacing UFO """
        color = RED
        if self.hit_flash_timer > 0:
            color = WHITE # Flash white when hit
...
# ... (This function is updated) ...
    ...
    def start_new_game(self):
        ...
        self.game_start_timer = 180 # NEW (3 seconds)
        self.ufo_spawn_timer = random.randint(UFO_SPAWN_TIME_MIN, UFO_SPAWN_TIME_MAX)
    ...
    def spawn_asteroids(self, count, level):
        ...
        if self.level > 1 and self.level % 4 == 0:
            is_minefield = True
            count = count // 2 # Less asteroids
            num_mines = min(2 + (self.level // 4), 6) # 2, 3, 4... up to 6 mines
        ...
        # NEW: Spawn mines (normal)
        if not is_minefield and random.random() < HUNTER_MINE_SPAWN_CHANCE * level:
             while True:
...
# ... (This function is unchanged) ...
                    ...
                    if (event.key == pygame.K_SPACE or event.key == pygame.K_z):
                        if len(self.bullets) < MAX_BULLETS or self.player.flow_state_timer > 0:
                            ...
                            if new_bullets:
                                ...
                                if self.player.flow_state_timer > 0: # NEW
                                    self.screen_shake_timer = 2 # NEW: Add laser "kick"
    ...
    def update(self):
        ...
        if self.game_start_timer > 0:
            ...
            self.all_sprites.update() # Let asteroids move/spawn for effect
            self.shockwaves.update()
            self.debris.update()
            return # Skip all gameplay logic
        ...
        if self.level_clear_timer > 0:
            ...
            self.particles = [p for p in self.particles if p.lifespan > 0]
            self.floating_texts.update()
            self.all_sprites.update() # Update sprites for spawn-in etc.
            self.shockwaves.update()
            self.debris.update()
        ...
        self.floating_texts.update() # <-- BUG FIX: This was missing
        ...
        # Allow more UFOs on screen at later levels
        max_ufos = 1 + (self.level // 5)
        if self.ufo_spawn_timer <= 0 and len(self.ufos) < max_ufos:
            # NEW: Spawn Elite UFO
            if self.level > 3 and random.random() < 0.4: # 40% chance for Elite
    ...
    def check_collisions(self):
        ...
        for ufo, bullets_hit in ufo_hits.items():
            if bullets_hit[0].is_laser: 
                ufo.health = 0 # Lasers one-shot
            else: 
                ufo.health -= 1
            ...
            if ufo.health <= 0:
                ...
                # Elite UFOs are worth more
                if isinstance(ufo, UFOElite):
                    final_score += self.player.add_score(SCORE_UFO, self.sounds) # Double score
    ...
    def draw(self):
        if self.game_state == "PLAYING":
            ...
            if self.game_start_timer > 0:
                ...
                text_str = f"{sec}"
                if self.game_start_timer < 40:
                    text_str = "GO!"
                
                # Scale text
                pulse = (self.game_start_timer % 60) / 60.0 # 1.0 -> 0.0
            ...
            # Draw "Level Clear"
            if self.level_clear_timer > 0 and self.game_start_timer == 0: # Don't show at same time
            ...
            # NEW: Add engine rumble
            thrust_shake_offset = (0, 0)
            if self.player.thrusting and self.game_start_timer == 0:
            ...
            self.screen.blit(self.game_surface, final_offset)
